...
import bert_fine_bpr_mooc

config = Config()

batch_size = 40  # fined: 24, frozon all: 40
epochs = 5  #10
...
logger = logger_config(log_path='log_mooc30_allFrozen.txt', logging_name='train_and_test')

############################## PREPARE DATASET ##########################
begin_time = time.time()
train_data, test_data, user_num ,item_num, train_mat = data_utils_bbpr.load_all(user_num=83497,dataset='mooc') # mooc:83497
# item_num: 1574, actual item num: 1326. By ng_sample, actual item_j num: about to 1573.
# train_data: list of [u,i], length: 3779975, e.g. [[0, 1505], [0, 605], [0, 922], ...,[166378,901], [166378,185]]
# test_data:  list of [u,i], length: 1045971
# train_mat: (166379, 1590), element is 1 if u interact i, otherwise 0.
print('Data finished!', len(train_data), len(test_data), user_num, item_num, train_mat.shape)

piece_text_list, piece_len_list = data_utils_bbpr.obtain_text_first2_510(dataset='mooc')  # 只包括课程文本片段，每门课程不超过前2片段组成
# print('piece_text_list:', len(piece_text_list), ' total piece:', sum(piece_len_list)) # piece_text_list: 24915 total piece: 25881
all_inputs_ids, all_token_type_ids, all_attention_mask = data_utils_bbpr.get_piece_texts_tokenize(dataset='mooc')
all_tokens = (all_inputs_ids, all_token_type_ids, all_attention_mask)
print('all_inputs_ids shape:', all_inputs_ids.shape)

users_byitem = data_utils_bbpr.obtain_users_bytext(83497, dataset='mooc')  # mooc: 83497
...
for epoch in range(1, epochs): # epochs
    ...
    for user, item_i, item_j in train_loader:
        ...
        user_inputs, bs, user_pos, user_byitem_unique, user_piece_lens, itemi_pos, itemj_input, itemj_piece_len = \
            data_utils_bbpr.triplet_bertInputs_first2_510(user, item_i, item_j, users_byitem, piece_len_list, all_tokens)
    ...
    #
        ...
        # print('user_piece_lens:', user_piece_lens)
        # print('itemi_pos:', itemi_pos)
        # print('itemj_input:', itemj_input['input_ids'])
        # print('itemj_piece_len:', itemj_piece_len)

        itemi_represent, _, prediction_i, prediction_j = \
            model(user_inputs, bs, user_pos, user_byitem_unique, user_piece_lens, itemi_pos, itemj_input, itemj_piece_len)
        loss = -(prediction_i - prediction_j).sigmoid().log().sum()

        if count == 1:  # mooc
            print('First batch loss:', loss)
            print('itemi_represent for 524:', itemi_represent[0,:5])
    ...
    # # 一轮训练结束， 保存训练好的模型参数
    torch.save(model, 'bert-model/bert_bpr_mooc_' + str(epoch) + '.pt')  # BERT中所有层都冻结的模型

    print('test...')
    # 载入训练好的模型
    model_load = torch.load('bert-model/bert_bpr_mooc_' + str(epoch) + '.pt')
    model_load.cuda()
    ...
    #
    # （2） 编程进行测试
    texts_represent = evaluate.get_piece_text_represent(None, model_load, piece_text_list, tokenizer)
